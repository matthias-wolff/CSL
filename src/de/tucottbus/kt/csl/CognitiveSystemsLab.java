...
public class CognitiveSystemsLab
{
  ...
  private CognitiveSystemsLab(String[] args)
  {
    ...
    if (LCARS.getArg(args,"--nospeech")==null)
    {
      ...
      try
      {
        config.setProperty("data.feainfo",LCARS.getResourceFile(resPckg,"feainfo.object").getAbsolutePath());
        config.setProperty("data.sesinfo",LCARS.getResourceFile(resPckg,"sesinfo.object").getAbsolutePath());
        config.setProperty("data.gmm"    ,LCARS.getResourceFile(resPckg,"3_15.gmm"      ).getAbsolutePath());
        config.setProperty("data.dialog" ,LCARS.getResourceFile(resPckg,"dialog.fst"    ).getAbsolutePath());
        config.setProperty("data.vadinfo",LCARS.getResourceFile(resPckg,"3_10_mod.vad"  ).getAbsolutePath());
        config.setProperty("fst.sleep"   ,"5");
        
        String script = LCARS.getResourceFile(spePckg,"postproc.xtp").getAbsolutePath().replace('\\','/');
        String sesinf = config.getProperty("data.sesinfo").replace('\\','/');
        String recognizerExe = UasrSpeechEngine.findRecognizerExe().getAbsolutePath().replace('\\','/');
        String dLabProExe = recognizerExe.replaceAll("/recognizer.","/dlabpro.");
        Log.info("recognizer is: "+recognizerExe);
        Log.info("dLabPro is   : "+dLabProExe);
        config.setProperty("postproc.cmd",dLabProExe+" \""+script+"\" gui \""+sesinf+"\" ");

        // TODO This takes very long! -->
        Log.info("Seeking audio device \"MADI (29+30)\" ...");
        int audioDeviceId = Recognizer.getAudioDeviceIdForName(new File(recognizerExe),"MADI (29+30)");
        if (audioDeviceId>=0)
        {
      ...
      {
        Log.err("Failed to start UASR speech engine, reason: "+e.getMessage(),e);
  ...
  {
    if (instance==null) instance = new CognitiveSystemsLab(null);
    return instance;
  }
  ...
  /**
   * The CSL main method. Use script <code>csl.bat</code> in the
   * project root folder to start CSL from the command line.
   * 
   * @param args
   *  Command line options, including:<pre>
   ...
   *  [2] The LCARS server panel is show by default</pre> 
   *  All {@linkplain LCARS#main(String[]) LCARS command line options} may be
   *  used, too. 
   */
